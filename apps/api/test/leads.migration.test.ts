  ...
  it.each([
    {
      ...
      nextVersion: '2026-09-24',
      priorMigrations: [],
      migration: '0003_privacy_policy_20260924.sql',
    },
    {
      previousVersion: '2026-09-24',
      nextVersion: '2026-09-26',
      priorMigrations: ['0003_privacy_policy_20260924.sql'],
      migration: '0004_privacy_policy_20260926.sql',
    },
  ])(
    'preserves history when moving from $previousVersion to $nextVersion',
    async ({ previousVersion, nextVersion, priorMigrations, migration }) => {
      const client = new PGlite();
      try {
        for (const file of [
      ...
      `);
        ...
        const beforeOutbox = await client.query('SELECT * FROM notification_outbox');
        expect(beforeLead.rows[0]).toMatchObject({ consent_version: previousVersion });

        await client.exec(await readFile(resolve(migrations, migration), 'utf8'));
      ...
      `);
        ...
        ).toEqual([{ consent_version: nextVersion }]);
      } finally {
        await client.close();
      }
    },
