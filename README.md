# Сладкий Дар — оптовые подарочные наборы с продукцией Kinder
...
«Сладкий Дар» — рабочее название сервиса оптовой продажи подарочных наборов с продукцией Kinder. Красно-белая палитра и предметная композиция опираются на изображение пользователя; кейсы, изображения и коммерческие формулировки — демонстрационное наполнение. Пользователь не предоставил реальные реквизиты, контакты и кейсы: замените их перед публикацией. Сторонние логотипы, фотографии и тексты референса не копируются. Композиционные наблюдения — в [reference audit](docs/reference-audit.md), технические решения — в [архитектуре](docs/architecture.md).
...
Технические поля `pageUrl`, `source`, `referrer`, UTM, `clientRequestId`, honeypot и обязательное `consent: true` сохраняются. Значение `consent_version = 2026-09-10` обозначает текущую версию согласия и не изменяется при редактировании оформления сайта.
...
Внутренние имена Compose-проекта, Docker images, баз и workspace пока сохраняют технический идентификатор `povod`. Пользовательское название — «Сладкий Дар»; смена названия сайта не переименовывает существующие volumes и не создаёт новую пустую базу.
